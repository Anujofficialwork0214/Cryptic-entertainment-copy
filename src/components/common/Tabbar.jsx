    ...
    <nav className="flex md:hidden flex-row items-center justify-between px-8 h-16  bg-white dark:bg-gray-900 visible md:invisible fixed bottom-0 w-full rounded-t-3xl text-2xl z-20">
      {navigationData.map((item, index) => (
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? " bg-gradient-to-t from-white/0 dark:from-black/0  to-white-500/20 dark:to-white-500/20 border-t-2 border-black text-black dark:text-white w-16 h-full flex items-center justify-center dark:border-white"
      ...
      <div className="flex justify-end gap-2  ">
        <div className="absolute -top-[92vh]  transform -translate-x-1/4  py-4 right-0 ">
          <a
