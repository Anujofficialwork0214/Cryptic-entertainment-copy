...
//     const yt = {};
//     data.forEach(person => {
//       if (person.instagramEmbed) insta[`insta-${person.id}`] = true;
//       if (person.youtubeEmbed) yt[`yt-${person.id}`] = true;
//     });
//     return { ...insta, ...yt };
//   });
...
//   // Process Instagram embeds after every render where loadingStates changes
//   useEffect(() => {
...
//     data.forEach(person => {
//       const ytId = `yt-${person.id}`;
//       const instaId = `insta-${person.id}`;

//       if (person.youtubeEmbed) {
//         const interval = setInterval(() => {
//           const iframe = document.querySelector(`iframe[src="${person.youtubeEmbed}"]`);
//           if (iframe) {
//             setLoadingStates(prev => ({ ...prev, [ytId]: false }));
...
//         const interval = setInterval(() => {
//           const container = document.getElementById(instaId);
//           const iframe = container?.querySelector('iframe');
//           if (iframe) {
//             setLoadingStates(prev => ({ ...prev, [instaId]: false }));
//             clearInterval(interval);
//           }
//         }, 300);
//         intervals.push(interval);
  ...
  return (
    <div className="snap-y snap-mandatory">
      {data.map((person) => {
        ...
        const ytId = `yt-${person.id}`;
        const showSkeleton =
          (person.youtubeEmbed && loadingStates[ytId]) ||
          (person.instagramEmbed && loadingStates[instaId]);
