...
const PastWork = () => {
    ...
    useEffect(() => {
        const fetchVideos = async() => {
            const response = await api.get('latestVideos');
    ...
    return (
        <>
            ...
            <div className="md:py-[100px] md:px-20 px-5 py-5 bg-slate-200 dark:bg-gray-800">
                ...
                >
                    {data.map((item, idx) => <VideoCard key={idx} title={item.title} thumbnail={item.thumbnail} videoId={item.videoId} />)}
                </Carousel>
...
const VideoCard = ({ thumbnail, videoId, title}) => {
    return (
        <Link to={`/video/${videoId}`} className="p-4 block">
